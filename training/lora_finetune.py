"""
LoRA finetuning script for Shakespeare dataset.
...
import torch
from config import get_lora_config, print_config
...
def load_model_and_tokenizer(model_name: str) -> tuple[GPT2LMHeadModel, GPT2Tokenizer]:
    ...
    print(f"Base model loaded with {model.num_parameters():,} parameters")
    return model, tokenizer


def setup_lora_model(model: GPT2LMHeadModel, config) -> GPT2LMHeadModel:
...
def plot_training_curves(output_dir: str) -> None:
    ...
    fig.suptitle("LoRA Finetuning Training Progress", fontsize=16, fontweight="bold")
    ...
    plot_path = os.path.join(plots_dir, "lora_finetune_training_curves.png")
    plt.savefig(plot_path, dpi=300, bbox_inches="tight", facecolor="white")
    print(f"Comprehensive LoRA training plots saved to: {plot_path}")
    ...
    plt.title(
        "LoRA Finetuning: Training vs Validation Loss", fontsize=14, fontweight="bold"
    )
    plt.legend(fontsize=12)
    plt.grid(True, alpha=0.3)
    
    simple_plot_path = os.path.join(plots_dir, "lora_training_loss_simple.png")
    plt.savefig(simple_plot_path, dpi=300, bbox_inches="tight", facecolor="white")
    print(f"Simple LoRA loss plot saved to: {simple_plot_path}")
...
def train_model(config) -> None:
    ...
    print("Creating LoRA training plots...")
